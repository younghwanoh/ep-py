...
SP = ep.SubPlotter(5, width=10, height=15, sharex=True)
...
LP = ep.LinePlotter(axis=SP.getAxis(4), title="Total bandwidth",
    xlabel="Time (ms)", ylabel="Bandwidth (GB/sec)")
LP.setFigureStyle(markersize=3)
...
LP.draw(BWdata)
LP.finish()
plt.xlim([0,stage_min_max.values()[-1][-1]*1.05]) # Manual hack of subplot xlim
